...
<body>
  <div class="content-page">
      <div class="content-page-nav">
          <a class="nav-list"><i class="icon index-icon pull-left"></i>首页</a><a class="nav-list"> > 文章库管理 </a><a class="nav-list"> > 点赞列表 </a>
      </div>
      <form action="<?=links('dz','index')?>" method="post">
      <div class="content-page-select-group row">
        ...
        <div class="col-md-1">
            <select class="content-page-select" name="ziduan" >
              <option value="tid"<?php if($ziduan=='tid') echo ' selected';?>>文章ID</option>
        			...
        			<option value="id"<?php if($ziduan=='id') echo ' selected';?>>点赞ID</option>
        ...
        <div class="col-md-1">
            <button class="content-page-btn blue-white-btn">搜点赞</button>
        </div>
      </div>
      </form>
      <form action="<?=links('dz','del',0,'ac=all')?>" method="post" id="myform" name="myform">
          ...
          <div class="content-page-table">
              ...
              <table class="table table-striped no-img-table">
                  ...
                  <tr>
                      ...
                      <th class="small-hide">用户ID</th>
                      <th class="small-hide">文章ID</th>
            			    <th class="small-hide">用户昵称</th>
                      <th class="w10 small-hide">点赞时间</th>
...
function del(obj,id){
    layer.confirm('删除后不能恢复，确认要删除吗？',function(index){
        $.post('<?=links('dz','del')?>',{id: id},function(data) {
           var msg=data['error'];
	   if(msg == "ok"){//成功
