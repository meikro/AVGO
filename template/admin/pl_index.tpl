...
<body>
  <div class="content-page">
      <div class="content-page-nav">
          ...
          </i>首页</a><a class="nav-list"> > 运营管理 </a><a class="nav-list"> > 评论列表 </a>
      </div>
      <form action="<?=links('pl','index')?>" method="post">
      <div class="content-page-select-group row">
        ...
        <div class="col-md-1">
            <select class="content-page-select" name="ziduan" >
              <option value="id"<?php if($ziduan=='id') echo ' selected';?>>评论ID</option>
        			<option value="uid"<?php if($ziduan=='uid') echo ' selected';?>>用户ID</option>
        			<option value="did"<?php if($ziduan=='tid') echo ' selected';?>>视频ID</option>
        			<option value="text"<?php if($ziduan=='text') echo ' selected';?>>评论内容</option>
            </select>
        </div>
        <div class="col-md-1">
        ...
        <div class="col-md-1">
            <button class="content-page-btn blue-white-btn">搜评论</button>
        </div>
      </div>
      </form>
      <form action="<?=links('pl','del',0,'ac=all')?>" method="post" id="myform" name="myform">
          ...
          <div class="content-page-table">
              ...
              <table class="table table-striped no-img-table">
                  ...
                  <tr>
                      ...
                      <th class="w30">视频标题</th>
                      <th class="w35">评论内容</th>
                      <th class="small-hide">用户昵称</th>
                      <th class="w10 small-hide">评论时间</th>
                  ...
                  <tbody>
                    ...
                    foreach ($array as $row) {
                          echo '
                            ...
                            <td class="w20"><a href="//'.Web_Url.links('show','index',$row->did,0,1).'" target="_blank"><u class="text-primary">'.getzd('vod','name',$row->did).'</u></a></td>
                            <td class="w30">'.$row->text.'</td>
                    				<td class="small-hide">'.getzd('user','nichen',$row->uid).'</td>
...
function del(obj,id){
    layer.confirm('删除后不能恢复，确认要删除吗？',function(index){
        $.post('<?=links('pl','del')?>',{id: id},function(data) {
           var pl=data['error'];
	   if(pl == "ok"){//成功
