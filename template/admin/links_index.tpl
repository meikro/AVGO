...
<body>
  <div class="content-page">
      <div class="content-page-nav">
          ...
          </i>首页</a><a class="nav-list"> > 运营管理 </a><a class="nav-list"> > 友情链接列表 </a>
      </div>
      <form action="<?=links('links','del',0,'ac=all')?>" method="post" id="myform" name="myform">
      <div class="content-page-table">
          <div class="content-page-table">
              <div class="content-page-table-top">
                  <div class="pull-left">
										<a class="gop" href="javascript:;" onclick="cmd('添加友情链接','<?=links('links','edit')?>')">
											<i class="icon table-add-all-icon pull-left"></i>添加友情链接</a>
                  </div>
                  ...
                  <tbody>
                    ...
                    if(empty($array)) echo '<tr><td colspan="7" style="text-align:center">没有记录~</td></tr>';
										foreach ($array as $row) {
										        $pic = empty($row->pic) ? '--' : '<img src="'.$row->pic.'">';
										        $lx = ($row->cid==0) ? '<span class="label label-success radius">文字</span>' : '<span class="label label-primary radius">图片</span>';
										                  echo '
													<tr>
                    				...
                    				<td class="table-last-group">
															<a style="text-decoration:none" onClick="cmd(\'编辑修改\',\''.links('links','edit',0,'id='.$row->id).'\')" href="javascript:;" title="编辑">
...
function del(obj,id){
    layer.confirm('删除后不能恢复，确认要删除吗？',function(index){
        $.post('<?=links('links','del')?>',{id: id},function(data) {
           var msg=data['error'];
    	   if(msg == "ok"){//成功
