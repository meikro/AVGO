...
<body>
  <div class="content-page">
      <div class="content-page-nav">
          ...
          </i>首页</a><a class="nav-list"> > 运营管理 </a><a class="nav-list"> > 页面列表 </a>
      </div>
      <form action="<?=links('pages','del',0,'ac=all')?>" method="post" id="myform" name="myform">
      <div class="content-page-table">
          <div class="content-page-table">
              <div class="content-page-table-top">
                  <div class="pull-left">
										<a class="gop" href="javascript:;" onclick="cmd('添加页面','<?=links('pages','edit')?>')">
											<i class="icon table-add-all-icon pull-left"></i>添加页面</a>
                  </div>
                    ...
                    if(empty($array)) echo '<tr><td colspan="7" style="text-align:center">没有记录~</td></tr>';
										foreach ($array as $row) {
										        ...
										        <td class="tab-checkbox"><input type="checkbox" value="'.$row->id.'" name="id[]"></td>
														...
														<td><a href="'.links('pages','index',$row->bs,'',1).'" target="_blank">'.$row->name.'</a></td>
														<td class="small-hide">'.sub_str($row->text,50).'</td>
														<td>'.$row->bs.'</td>
														<td class="small-hide">'.$zt.'</td>
                    				<td class="table-last-group">
															<a style="text-decoration:none" onClick="cmd(\'编辑修改\',\''.links('pages','edit',0,'id='.$row->id).'\')" href="javascript:;" title="编辑">
...
function del(obj,id){
    layer.confirm('删除后不能恢复，确认要删除吗？',function(index){
        $.post('<?=links('pages','del')?>',{id: id},function(data) {
           var msg=data['error'];
	   if(msg == "ok"){//成功
