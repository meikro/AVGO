...
<body>
  <div class="content-page">
      <div class="content-page-nav">
          <a class="nav-list"><i class="icon index-icon pull-left"></i>首页</a><a class="nav-list"> > 视频库管理 </a><a class="nav-list"> > 专题列表 </a>
      </div>
      ...
      <div class="content-page-select-group row">
        ...
        <div class="col-md-1">
            <select class="content-page-select" name="ziduan" >
              <option value="name"<?php if($ziduan=='name') echo ' selected';?>>专题名称</option>
              <option value="id"<?php if($ziduan=='id') echo ' selected';?>>专题ID</option>
            </select>
        </div>
        <div class="col-md-1">
        ...
        <div class="col-md-1">
            <button class="content-page-btn blue-white-btn">搜专题</button>
        </div>
      </div>
      </form>
      <form action="<?=links('topic','del',0,'ac=all')?>" method="post" id="myform" name="myform">
      <div class="content-page-table">
          <div class="content-page-table">
              <div class="content-page-table-top">
                  <div class="pull-left">
                      <a class="gop" href="javascript:;" onclick="cmd('添加专题','<?=links('topic','edit')?>')">
                        <i class="icon table-add-all-icon pull-left"></i>添加专题</a>
                  </div>
              ...
              <table class="table table-striped img-table">
                  ...
                  <tbody>
                    <?php
                    if(empty($array)) echo '
                    <tr><td colspan="7" style="text-align:center">没有记录~</td></tr>';
                        ...
                        if(date('Y-m-d')==$time) $time = '<font color=red>'.$time.'</font>';
                            ...
                            <td class="tab-checkbox">
                              <input type="checkbox" value="'.$row->id.'" name="id[]">
                    				...
                    				<td><a href="http://'.Web_Url.links('topic','show',$row->id,'',1).'" target="_blank"><u class="text-primary">'.$row->name.'</u></a></td>
                    				<td class="small-hide">'.$row->hits.'</td>
                    				<td class="small-hide">'.$time.'</td>
                    				<td class="table-last-group">
                    				    <a style="text-decoration:none"  onClick="cmd(\'专题编辑\',\''.links('topic','edit',0,'id='.$row->id).'\')" href="javascript:;" title="编辑">
...
function del(obj,id){
    layer.confirm('删除后不能恢复，确认要删除吗？',function(index){
        $.post('<?=links('topic','del')?>',{id: id},function(data) {
           var msg=data['error'];
	   if(msg == "ok"){//成功
