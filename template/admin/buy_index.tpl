...
<body>
  <div class="content-page">
      <div class="content-page-nav">
          ...
          </i>首页</a><a class="nav-list"> > 会员管理 </a><a class="nav-list"> > 消费列表 </a>
      </div>
      <form action="<?=links('buy','index')?>" method="post">
      ...
      <form action="<?=links('buy','del',0,'ac=all')?>" method="post" id="myform" name="myform">
      <div class="content-page-table">
          <div class="content-page-table">
              <div class="content-page-table-top">
                  <div class="pull-left">
                    <span class="l">&nbsp;<font color=red>注意：消费记录删除后，会员观看视频会重新计费，请慎重删除记录~</font></span>
                  </div>
                        ...
                        if(date('Y-m-d')==date('Y-m-d',$row->addtime)) $time = '<font color=red>'.$time.'</font>';
                          ...
                          echo '
                            ...
                            <td class="tab-checkbox"><input type="checkbox" value="'.$row->id.'" name="id[]"></td>
                    				...
                    				<td><a href="'.links('show','index',$row->did,0,1).'" target="_blank"><u class="text-primary">'.$name.'</u></a></td>
...
function cmd(title,url){
    layer_show(title,url,400,280);
}
/*删除*/
function del(obj,id){
    layer.confirm('删除后不能恢复，确认要删除吗？',function(index){
        $.post('<?=links('buy','del')?>',{id: id},function(data) {
           var msg=data['error'];
	   if(msg == "ok"){//成功
