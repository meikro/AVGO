...
if(VOD_JM==1){
	if(strpos($url,'~') !== FALSE){
		$uarr = explode('~',$url);
		$url = sys_auth($uarr[0]);
		if(!empty($uarr[1])) $url.= '~'.$uarr[1];
	}else{
		$url = sys_auth($url);	
		if(strpos($url,'&type=') !== FALSE){
			$path = end(explode('&type=',$url));
		}
	}
}
//兼容老版本
if(!empty($url) && !empty($path)) $url.='~'.$path;
//奇艺
$qy = ($_GET['type']=='qiyi' || strpos($url,'qiyi.com') !== false) ? 1 : 0;
?>
<!DOCTYPE html>
...
<script type="text/javascript" src="<?php echo WEB_PATH;?>ckplayer/yunparse.js?v=1.29"></script>
