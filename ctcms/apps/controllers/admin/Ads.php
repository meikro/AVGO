...
class Ads extends Ctcms_Controller {
	...
	public function save()
	{
		...
		$data['neir'] = str_encode($neir);
		$data['name'] = $this->input->post('name',true);
		$data['bs'] = $this->input->post('bs',true);
		$data['yid'] = (int)$this->input->post('yid',true);
		if(empty($data['name']) || empty($data['bs'])){
             admin_msg('数据不完整~！','javascript:history.back();','no');
		}
		if($id==0){
            $id = $this->csdb->get_insert('ads',$data);
		}else{
            $this->csdb->get_update('ads',$id,$data);
		}
		//写入JS
		$jspath = FCPATH.'attachment/js/'.$id.'.js';
		if($data['yid'] == 1){
			$neir = '';
		}
		$js = htmltojs($neir);
		write_file($jspath,$js);
        echo "<script>
