...
class Dz extends Ctcms_Controller {
	...
	public function index()
	{
        ...
        //总数量
	    $total = $this->csdb->get_nums('dz',$where,$like);
		//每页数量
	    $per_page = 15; 
		//总页数
        ...
        //记录数组
	    $data['array'] = $this->csdb->get_select('dz','*',$where,'addtime DESC',$limit,$like);
		...
		//当前链接
		$base_url = links('dz','index',0,'ziduan='.$ziduan.'&key='.urlencode($key).'&kstime='.$kstime.'&jstime='.$jstime);
		
		//分页
	    $data['pages'] = admin_page($base_url,$total,$pagejs,$page);  //获取分页类
	    $data['nums'] = $total;
		$this->load->view('head.tpl',$data);
		$this->load->view('dz_index.tpl');
	}
	...
	public function del()
	{
 	    ...
 	    $query = $this->csdb->get_row('dz','*',$id);
		$res=$this->csdb->get_del('dz',$id);
		if($res){
			 $this->db->query("update ".CT_SqlPrefix."comm set dznum=dznum-1 where id=".$query->tid."");
		}
		if($ac=='all'){
			if($res){
                admin_msg('恭喜您，删除完成~！',links('dz'));
			}else{
