...
class Msg extends Ctcms_Controller {
	...
	public function index()
	{
        ...
        //总数量
	    $total = $this->csdb->get_nums('msg',$where,$like);
		//每页数量
	    $per_page = 15; 
		//总页数
        ...
        //记录数组
	    $data['array'] = $this->csdb->get_select('msg','*',$where,'addtime DESC',$limit,$like);
	    foreach ($data['array'] as $k => $value) {
	    	...
	    	//获取作者昵称
	    	if($value->ruid == 0){
	    		$value->rnichen = '对文章的回复';
	    	}else{
	    		$query1 = $this->csdb->get_row('user','*',$value->ruid);
		    	if(empty($value->rnichen)){
		    		$value->rnichen = $query1->name;
		    	}else{
		    		$value->rnichen = $query1->nichen;
		    	}
	    	}
		...
		//当前链接
		$base_url = links('msg','index',0,'ziduan='.$ziduan.'&key='.urlencode($key).'&kstime='.$kstime.'&jstime='.$jstime);
		
		//分页
	    $data['pages'] = admin_page($base_url,$total,$pagejs,$page);  //获取分页类
	    $data['nums'] = $total;
		$this->load->view('head.tpl',$data);
		$this->load->view('msg_index.tpl');
	}
	...
	public function del()
	{
 	    ...
 	    $query = $this->csdb->get_row('msg','*',$id);
		$res=$this->csdb->get_del('msg',$id);
		if($res){
			if($query->rid == 0){
