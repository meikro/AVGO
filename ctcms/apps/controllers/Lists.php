...
class Lists extends Ctcms_Controller {
	...
	public function index($cid=0,$page=1)
	{
	    ...
	    if(!($this->cache->start($cache_id))){
				...
				if(!empty($row['title'])) $data['ctcms_title'] = $row['title'];
				if(!empty($row['keywords'])) $data['ctcms_keywords'] = $row['keywords'];
				if(!empty($row['description'])) $data['ctcms_description'] = $row['description'];
				//当前数据
				...
				//当前ID
				$data['ctcms_cid']=$cid;
				$data['ctcms_fid'] = $row['fid'] == 0 ? $cid : $row['fid'];
